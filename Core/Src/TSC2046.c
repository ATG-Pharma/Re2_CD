...
void Print_Cal_Vals(void){
	char Scale_X_Text[256];
	char Scale_Y_Text[256];
	char Bias_X_Text[256];
	char Bias_Y_Text[256];
	snprintf(Scale_X_Text, sizeof(Scale_X_Text), "ScaleX: %f", myTS_Calibrate.Scale_X);
	snprintf(Scale_Y_Text, sizeof(Scale_Y_Text), "ScaleY: %f", myTS_Calibrate.Scale_Y);
	snprintf(Bias_X_Text, sizeof(Bias_X_Text), "BiasX: %f", myTS_Calibrate.Bias_X);
	snprintf(Bias_Y_Text, sizeof(Bias_Y_Text), "BiasY: %f", myTS_Calibrate.Bias_Y);
	ILI9341_printText(Scale_X_Text, 20, 20, COLOR_RED, COLOR_WHITE, 2);
	ILI9341_printText(Scale_Y_Text, 20, 40, COLOR_RED, COLOR_WHITE, 2);
	ILI9341_printText(Bias_X_Text, 20, 60, COLOR_RED, COLOR_WHITE, 2);
	ILI9341_printText(Bias_Y_Text, 20, 80, COLOR_RED, COLOR_WHITE, 2);
}

void Set_Cal_Vals(void){
	myTS_Calibrate.Scale_X = SCALE_X_VALUE;
	myTS_Calibrate.Scale_Y = SCALE_Y_VALUE;
	myTS_Calibrate.Bias_X = BIAS_X_VALUE;
	myTS_Calibrate.Bias_Y = BIAS_Y_VALUE;
